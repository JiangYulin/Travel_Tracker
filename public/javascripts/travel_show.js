...
window.onload = function() {
    $.ajax({
        type: "POST",
        url:"/travel/"+$("#travelID").attr("travel_id")+"/comments",
        context: $("#comments_container").find(".travel-comments"),
...
$(".btn-commit").click(get_commit);
function get_commit(event) {
    event.preventDefault();
    var ele = $(this).closest(".travel-img-container").find(".travel-comments-container");
    if(ele.is(":visible")) {
        ele.hide();
        return ;
    };
    ...
    $.ajax({
        ...
        context: ele.find(".travel-comments"),
        success: function(data) {
            console.log(data);
            var li_ele,
                span_ele,
...
$(".form-submit").click(function(event) {
    ...
    var form = $(this).closest(".travel-comment-form");
    ...
    $.ajax({
        ...
        data    : data,
        context : form.closest(".travel-comments-container"),
        contentType: false,
        processData: false,
        success : function(data) {
            console.log(data);
            $(this).find(".travel-comment-form")[0].reset();
            ...
            li_ele.appendChild(a_ele);
            li_ele.appendChild(p_ele);
            $(this).find(".travel-comments")[0].appendChild(li_ele);
            /*
            这里重新绑定了一次函数，有点多余。
             */
            reply_to_user();
...
$(".reply-to-user").click(function(event){
    console.log("ok");
    event.preventDefault();
    console.log($(this).closest('.travel-comment').attr('user_id'));
    var user_id = $(this).closest('.travel-comment').attr('user_id');
    var nick_name = $(this).closest('.travel-comment').attr('nick_name');
    var form = $(this).closest(".travel-comments-container").find(".travel-comment-form");
...
$(".delete_user").click(function(event) {
    ...
    $(this).closest(".travel-comment-form").find("textarea").attr("placeholder","");
    $(this).closest(".travel-comment-form").find("input[name='to_user_id']").attr("value", "");
    $(this).closest(".travel-comment-form").find("input[name='to_user_nick_name']").attr("value", "");
})
...
$("#reply-to-travel").click(function(event) {
    event.preventDefault();
    var form = $("#travel-comment-form");
    ...
    $.ajax({
        type:"POST",
        url: "/travel/"+$("#travelID").attr("travel_id")+"/reply",
        data: data,
        context: $('#comments_container'),
        contentType:false,
        processData:false,
        success: function(data) {
            console.log(data);
            $("#travel-comment-form")[0].reset();
            var li = document.createElement("li");
            li_ele = document.createElement("li");
            span_ele = document.createElement("span");
